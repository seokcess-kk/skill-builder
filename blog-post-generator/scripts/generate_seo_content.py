...
def _parse_title_tags_body(content):
    ...
    for i, line in enumerate(lines):
        ...
        # TITLE1/TITLE2/TITLE3 행
        if re.match(r'^TITLE\d*:', stripped, re.IGNORECASE):
...
"""
    ...
    print(f"  내부 검증: {grade}등급 (issue {len(val_issues)}건, warning {len(val_warnings)}건)")

    if grade in ("C", "D") and val_issues:
        print("-" * 50)
        print("  등급 미달 — 자동 수정 시도 중...")
        ...
        fix_prompt = f"""아래는 SEO 블로그 원고입니다. 검증 결과 문제가 발견되었습니다.
문제를 해결하여 수정된 원고를 출력하세요.
...
"""
        ...
        if fix_result.returncode == 0:
            ...
            _fix_titles, _, fixed_body_clean = _parse_title_tags_body(fixed_body)
            if fixed_body_clean:
            ...
            if fixed_chars >= orig_chars * 0.8:
                ...
                if rank2 > rank1 or (rank2 == rank1 and issues2 >= len(val_issues)):
                    # 수정본이 원본보다 나쁘거나, 같은 등급인데 issues도 줄지 않았으면 원본 유지
