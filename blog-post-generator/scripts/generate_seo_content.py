...
def find_previous_seo_structures(output_dir, current_keyword=""):
    """이전 SEO 원고의 소제목 + 핵심 내용을 검색하여 반복 방지용 이력 반환

    output/{keyword-slug}/seo/content/seo-content.md 구조를 전제로 스캔.
    유사 키워드(공통 핵심어 포함)의 원고는 상세 내용까지 추출하여
    내용 차별화를 강화합니다.
    """
    ...
    # 현재 키워드에서 핵심어 추출 (지역명 제거 후 공통 주제어)
    # 예: "압구정울쎄라" → "울쎄라", "청담동울쎄라" → "울쎄라"
    core_keyword = _extract_core_keyword(current_keyword) if current_keyword else ""
    ...
    for content_file in sorted(output_root.glob("*/seo/content/seo-content.md")):
        ...
        prev_keyword = content_file.parent.parent.parent.name

        # 유사 키워드 판정: 핵심어가 겹치는지 확인
        prev_core = _extract_core_keyword(prev_keyword)
        is_similar = (core_keyword and prev_core and
                      (core_keyword in prev_core or prev_core in core_keyword))

        entry = {
            "keyword": prev_keyword,
            "headings": headings,
            "is_similar": is_similar,
        }

        # 유사 키워드면 섹션별 핵심 내용도 추출 (차별화 강화)
        if is_similar:
            section_summaries = _extract_section_summaries(text, headings)
            entry["section_summaries"] = section_summaries
...
def _extract_core_keyword(keyword):
    """키워드에서 지역명/접미사를 제거하고 핵심 주제어를 추출
    ...
    # 흔한 지역명 패턴 제거
    locations = [
        "강남", "압구정", "청담", "청담동", "신사", "신사역", "논현", "삼성",
        "서초", "잠실", "송파", "목동", "여의도", "홍대", "이태원", "성수",
        "강서", "마포", "영등포", "종로", "동대문", "수원", "분당", "판교",
        "일산", "부산", "대구", "대전", "광주", "인천", "창원", "천안",
        "전주", "울산", "제주", "세종", "고양", "성남", "부천", "안양",
        "의정부", "평택", "김포", "파주", "하남", "남양주", "구리",
        "동탄", "범일역", "수성구", "진구", "의창구", "부평구", "우만동",
        "장항동", "불당", "둔산동", "서구", "중구",
    ]
    core = keyword
    for loc in sorted(locations, key=len, reverse=True):
        core = core.replace(loc, "")
    core = core.strip()
    return core if core else keyword


def _extract_section_summaries(text, headings):
    """원고 텍스트에서 섹션별 핵심 1~2문장을 추출"""
    summaries = []
    # 이미지 마커 제거
    clean = re.sub(r'\[IMAGE:[^\]]+\]', '', text)

    for idx, heading in enumerate(headings):
        h_pos = clean.find(heading)
        if h_pos < 0:
            continue

        after = clean[h_pos + len(heading):]

        # 다음 소제목까지
        next_pos = len(after)
        for next_h in headings[idx + 1:]:
            np = after.find(next_h)
            if np >= 0:
                next_pos = np
                break

        section = after[:next_pos].strip()
        # 마크다운 제거
        ...
        for sent in sentences:
            ...
            char_sum += len(sent)
            if char_sum >= 120 or len(summary_parts) >= 2:
                break
...
def _build_writing_rules(writing_strategy, previous_structures=None):
    ...
    if previous_structures:
        # 유사 키워드(같은 주제)와 일반 키워드 분리
        similar_prevs = [p for p in previous_structures if p.get("is_similar")]
        other_prevs = [p for p in previous_structures if not p.get("is_similar")]

        # 유사 키워드 원고 — 상세 내용까지 회피 (핵심)
        if similar_prevs:
            lines.append("   **⚠ 같은 주제의 다른 키워드로 이미 작성된 원고가 있습니다. 반드시 내용을 차별화하세요:**")
            for prev in similar_prevs[:5]:
                lines.append(f"   - **'{prev['keyword']}'** 원고:")
                lines.append(f"     소제목: {' → '.join(prev['headings'][:5])}")
                for sec in prev.get("section_summaries", [])[:4]:
                    lines.append(f"     · [{sec['heading']}] {sec['summary'][:80]}")
            lines.append("   **차별화 방법:**")
            lines.append("   - 위 원고들과 다른 관점/각도에서 접근하세요 (예: 같은 '울쎄라'라도 시술 원리 vs 병원 선택 기준 vs 시술 후 관리 등)")
            lines.append("   - 소제목이 겹치지 않아야 합니다")
            lines.append("   - 도입부의 화두/질문이 달라야 합니다")
            lines.append("   - 같은 정보를 다루더라도 표현과 구성을 완전히 바꾸세요")

        # 일반 이전 원고 — 소제목 구조만 회피
        if other_prevs:
            avoid_lines = []
            for prev in other_prevs[-3:]:
...
def generate_content(keyword, analysis_path, output_dir):
    ...
    previous_structures = find_previous_seo_structures(output_dir, current_keyword=keyword)
...
"""
    ...
    if previous_structures:
        similar_count = sum(1 for p in previous_structures if p.get("is_similar"))
        other_count = len(previous_structures) - similar_count
        parts = []
        if similar_count:
            parts.append(f"유사 키워드 {similar_count}건 (내용 차별화)")
        if other_count:
            parts.append(f"기타 {other_count}건 (구조 회피)")
        print(f"  이전 원고 이력: {', '.join(parts)}")
    print("-" * 50)
